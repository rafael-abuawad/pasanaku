...
@external
def faucet(to: address, amount: uint256) -> bool:
    """
    @dev Mints `amount` tokens to `to`.
    @notice Note that `to` cannot be the zero address.
    @param to The 20-byte recipient address.
    @param amount The 32-byte token amount to be minted.
    @return True if the mint was successful.
    """
    assert to != empty(address), "erc20_mock: mint to the zero address"
